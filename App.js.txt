...
import { Camera, FileImage, Trash2, UserCog, X, Plus, Save, Loader2, ShieldCheck, AlertTriangle, KeyRound, Archive, ArrowLeft, Info, LogOut, Shield, FileDown } from 'lucide-react';

// NOTE: The following lines are for the PDF export functionality.
// To make this work, you must add these script tags to your public/index.html file, inside the <head> tag.
...
function MainApp({ user }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            showModal("Suksess!", "Rapporten er lagret i arkivet. Skjemaet vil nå nullstilles.", () => resetForm(false));
        } catch (error) {
        ...
        <div className="bg-gray-50 min-h-screen font-sans">
            ...
            <footer className="fixed bottom-0 left-0 right-0 bg-white bg-opacity-90 p-4 border-t z-10">
                <div className="container mx-auto flex flex-col md:flex-row gap-4">
                     <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="w-full md:w-2/3 bg-green-600 text-white font-bold py-3 px-4 rounded-md flex items-center justify-center disabled:bg-gray-400">
                        {isSubmitting ? <Loader2 className="animate-spin mr-2" /> : <Save size={22} className="mr-2" />}
                        {isSubmitting ? 'Lagrer...' : 'Lagre Rapport'}
                    </button>
...
function ReportDetailView({ report, onBack }) {
    ...
    const downloadPdf = async () => {
        ...
        if (!window.html2canvas || !window.jspdf) {
            ...
            alert("Kunne ikke generere PDF. Nødvendige biblioteker mangler. Sørg for at du har lagt til script-taggene i public/index.html.");
    ...
    return (
        <div className="overflow-y-auto flex-grow">
            ...
            <div ref={reportRef} className="bg-white p-4 sm:p-6 rounded-lg border">
